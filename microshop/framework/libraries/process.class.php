...
class processClass{
	...
	const MAX_LOGIN = 5;	//密码连续输入多少次被暂时锁定
	const MAX_COMMIT = 2;	//连续评论多少次被暂时锁定
	const MAX_REG = 5;		//连续注册多少个账号被暂时锁定
	...
	public static function parsekey($type){
		return str_replace(array('login','commit','reg'),array(1,2,3),$type);
	}
	...
	public static function parsetimes($type){
		return str_replace(array('login','commit','reg'),array(self::MAX_LOGIN ,self::MAX_COMMIT ,self::MAX_REG),$type);
	}
}
