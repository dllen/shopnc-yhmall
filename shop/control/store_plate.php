...
class store_plateControl extends BaseSellerControl {
    ...
    public function plate_listOp() {
        ...
        $plate_list = $store_plate->getPlateList($where, '*', 10);
    ...
    public function plate_addOp() {
        if (chksubmit()) {
            ...
            $result = Model('store_plate')->addPlate($insert);
            if ($result) {
    ...
    public function plate_editOp() {
        if (chksubmit()) {
            ...
            $where['store_id']  = $_SESSION['store_id'];
            $result = Model('store_plate')->editPlate($update, $where);
            if ($result) {
                showDialog(L('nc_common_op_succ'), urlShop('store_plate', 'index'),'succ');
            } else {
                showDialog(L('nc_common_op_fail'), urlShop('store_plate', 'index'));
        ...
        $plate_info = Model('store_plate')->getPlateInfo(array('plate_id' => $plate_id, 'store_id' => $_SESSION['store_id']));
