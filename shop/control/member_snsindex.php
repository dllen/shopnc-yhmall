...
class member_snsindexControl extends BaseMemberControl {
	...
	public function sharegoodsOp(){
		if ($_POST['form_submit'] == 'ok'){
			...
			$goods_info = $goods_model->getGoodsOnlineInfo($condition,'goods_id,goods_name,goods_image,goods_price,goods_freight,goods_collect,store_id,store_name');
			if (empty($goods_info)){
	...
	public function sharestoreOp(){
		if ($_POST['form_submit'] == 'ok'){
			...
			if ($result){
				...
				$content_str .= "<div class=\"fd-media\">
					<div class=\"goodsimg\"><a target=\"_blank\" href=\"{$store_info['store_url']}\"><img src=\"{$store_info['store_label']}\" onload=\"javascript:DrawImage(this,120,120);\" alt=\"{$store_info['store_name']}\"></a></div>
					<div class=\"goodsinfo\">
						<dl>
	...
	public function sharegoods_oneOp(){
		...
		//查询商品信息
		$goods_info = Model('goods')->getGoodsInfo(array('goods_id'=>$gid));
		//判断系统是否开启站外分享功能
		if (C('share_isuse') == 1){
			//站外分享接口
			$model = Model('sns_binding');
