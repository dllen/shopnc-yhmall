...
class store_activityControl extends BaseSellerControl {
	...
	public function store_activityOp(){
		...
		Tpl::output('menu_sign_url','index.php?act=store_activity&op=store_activity');
	...
	public function activity_applyOp(){
		...
		$gc_list	= $gc->getTreeClassList(3,array('order'=>'gc_parent_id asc,gc_sort asc,gc_id asc'));
	...
	public function activity_apply_saveOp(){
		...
		foreach ($_POST['item_id'] as $item_id){
			...
			if(!in_array($item_id,$ids)){
				...
				$goods	= Model('goods');
				$item	= $goods->getGoodsOnlineInfo($condition_goods, 'goods_name,store_id,store_name');
				if(empty($item)){
					continue;	
				}
