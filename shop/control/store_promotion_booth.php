...
class store_promotion_boothControl extends BaseSellerControl {
    ...
    public function booth_goods_listOp() {
        ...
        if (!empty($booth_quota)) {
            ...
            if (!empty($boothgoods_list)) {
                ...
                $goods_list = Model('goods')->getGoodsAsGoodsShowList(array('goods_id' => array('in', $goodsid_array)), 'goods_id,goods_name,goods_image,goods_price,store_id,gc_id');
                if (!empty($goods_list)) {
                    ...
                    $goodsclass_list = Model('goods_class')->getGoodsClassList(array('gc_id' => array('in', $gcid_array)), 'gc_id,gc_name');
    ...
    public function booth_quota_addOp() {
        if (chksubmit()) {
            ...
            if ($return) {
                ...
                $this->addcron(array('exetime' => $end_time, 'exeid' => $_SESSION['store_id'], 'type' => 4));
    ...
    private function profile_menu($menu_type,$menu_key='') {
        ...
        Tpl::output('menu_sign_url','index.php?act=store_promotion_booth');
