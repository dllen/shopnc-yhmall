    ...
    public function bundling_addOp() {
        ...
        if (chksubmit()) {
            ...
            if (!empty($_POST['goods']) && is_array($_POST['goods'])) {
                foreach ($_POST['goods'] as $key => $val) {
                    // 验证是否为本店铺商品
                    $goods_info = $model_goods->getGoodsInfo(array('goods_id' => $val['gid'], 'store_id' => $_SESSION['store_id']), 'goods_id,goods_name,goods_image');
                    if (empty($goods_info)) {
                        continue;
                    }
                    $data = array();
        ...
        if (intval($_GET['bundling_id']) > 0) {
            ...
            if (!empty($b_goods_list)) {
                ...
                $goods_list = Model('goods')->getGoodsAsGoodsShowList(array('goods_id' => array('in', $goodsid_array)), 'goods_id,goods_price,goods_image,goods_name');
