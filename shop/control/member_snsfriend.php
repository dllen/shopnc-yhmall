...
class member_snsfriendControl extends BaseMemberControl {
	...
	public function findOp(){
		if(chksubmit()) {
			...
			// 查看推荐
			$model = Model();
			$mtag_list = $model->table('sns_membertag')->where(array('mtag_recommend'=>1))->select();
			if(!empty($mtag_list)){
				// 查询已关注好友 ，不显示已关注好友
...
// 				var_dump($tagmember_list);die;
	...
	public function findlistOp(){
		if(trim($_POST['searchname']) != ''){
			...
			$memberlist = array();
			if($count > 0){
				$delaypage = intval($_POST['delaypage'])>0?intval($_POST['delaypage']):1;//本页延时加载的当前页数
