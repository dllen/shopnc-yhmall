...
class member_favoritesControl extends mobileMemberControl {
    ...
    public function favorites_addOp() {
		...
		$goods_info = $goods_model->getGoodsInfo(array('goods_id' => $goods_id));
		...
		if ($result){
			//增加收藏数量
			$goods_model->editGoods(array('goods_collect' => array('exp', 'goods_collect + 1')), array('goods_id' => $goods_id));
            output_data('1');
		}else{
            output_error('收藏失败');
		}
